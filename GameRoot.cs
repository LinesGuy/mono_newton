...
namespace mono_newton_directx
{
    public class myVector
    {
        public Complex X;
        public Complex Y;
        public myVector(Complex x, Complex y)
        {
            X = x;
            Y = y;
        }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            for (int y = 0; y < ScreenSize.Y; y += pixelSize)
            {
                for (int x = 0; x < ScreenSize.X; x += pixelSize)
                {
                    ...
                    /*
                    if (Vector2.Distance(coords, Vector2.Zero) < 1)
                        spriteBatch.Draw(Pixel, new Vector2(x, y), null, Color.Green, 0f, Vector2.Zero, pixelSize, SpriteEffects.None, 0);
                    
                    else
                        spriteBatch.Draw(Pixel, new Vector2(x, y), null, Color.White, 0f, Vector2.Zero, pixelSize, SpriteEffects.None, 0);
                    */
                }
            }
            spriteBatch.DrawString(DebugFont, Input.Mouse.Position.ToString(), Vector2.Zero, Color.Black);
            spriteBatch.DrawString(DebugFont, Camera.screen_to_world_pos(Input.Mouse.Position.ToVector2()).ToString(), new Vector2(0, 28), Color.Black);
            spriteBatch.End();
            base.Draw(gameTime);
        }
    }
